...
\*\*Phase 1 — Head training (epochs 1–10):\*\* Frozen MobileNetV2 base, train only the custom classification head with `learning\_rate=1e-3`. Reaches 94.6% validation accuracy.
...
!\[Training curves across both phases](notebooks/training\_curves\_combined.png)
...
!\[Sample predictions on validation set](notebooks/predictions\_sample.png)
...
│       ├── crop\_disease\_model.tflite   ← trained model (2.7 MB)

│       └── class\_names.json            ← 38 class labels

├── notebooks/

│   ├── training\_curves\_combined.png

│   └── predictions\_sample.png
...
\- \*\*iCassava dataset\*\* (Mwebaze et al.) for cassava diseases
